...
$fetch = $pdo->prepare('SELECT * FROM category ORDER BY cat_name ASC');
$fetch->execute();
                      ...
                      ?>
                        <tr class="">
                          ...
                          <td>
                            ...
                            <?php
                            $mfetdch = $pdo->prepare('SELECT * FROM course WHERE cat_id=:catimp AND Department=:depus ');
                            $mfetdch->bindValue(':catimp', $cap['cat_id']);
                            $mfetdch->bindValue(':depus', $cap['Department']);
                            $mfetdch->execute();
                            $mcatdep = $mfetdch->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($mcatdep as $mcap) {
                              echo $mcap['assigned_group'] . ',';
                            }
